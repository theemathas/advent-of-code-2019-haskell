-- Copied from q9/Part2.hs then edited
...
main = do
  ...
  print $ length $ touchedPanels $ runRobot program
...
runRobot program = go $ RobotState (runIntcodeProgram program) DirUp (0, 0) Set.empty Set.empty
  where
